...
public:
    ...
    ComponentPools& pools() { return scene.component_maps(); }
...
TEST_F(ComponentPoolsTest, AddOverExistingComponentPool)
{
    ...
    pools().get<TestComponent>()->add_param(EntityId(0u), 0);
    pools().get<TestComponent>()->add_param(EntityId(1u), 1);
...
public:
    ...
    void set_up() override { scene.component_maps().add<TestComponent>(); }
...
TEST_F(EntityTest, CloneEntity)
{
    ...
    assert_that(cloned_child->name(), equals(std::string("ChildTestEntity")));
    ...
    assert_that(cloned_child->get_component<TestComponent>()->x,
                equals(child->get_component<TestComponent>()->x));
    ...
    assert_that(cloned->get_component<TestComponent>()->x,
                equals(entity->get_component<TestComponent>()->x));
}