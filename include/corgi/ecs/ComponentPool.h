...
public:
	 ...
	 */
    [[nodiscard]] virtual EntityId entity_id(std::size_t component_index) const = 0;

    [[nodiscard]] virtual std::size_t entity_id_int(std::size_t component_index) const = 0;
	 ...
	 */
    virtual void*       at(std::size_t index)       = 0;
    virtual const void* at(std::size_t index) const = 0;
};
...
public:
    ...
    [[nodiscard]] bool contains(const EntityId id) const noexcept override
    {
        ...
        return _entity_id_to_components_vector[id.id_] != EntityId::npos;
    }
